...
export class TestProvider {
    ...
    return this.database
      ...
      .map((careers: any) => {
        ...
        careers.forEach(career => {
          const areas: Array<string> = career.areas;
          finalCareers.push({ match: this.getMatchCareers(areas, values), career });
        });
  ...
  private getMatchCareers(areas: Array<string>, values: Array<string>) {
    let match = 0;
